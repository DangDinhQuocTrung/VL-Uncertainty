...
class EvidenceModel:
    ...
    def get_evidence_ignorance(self):
        ...
        m_neg_omage = torch.exp(-torch.sum(self.w_neg1)) / 1 - torch.prod(
            1 - torch.exp(-self.w_neg1)
        )
    ...
    def get_nonspecific(self):
        ...
        m_theta = m_theta * (
            torch.exp(self.w_pos2)
            - 1
            + torch.prod(1 - torch.exp(-self.w_neg2)) / (1 - torch.exp(-self.w_neg2))
        )
