...
from custom_llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images_withclip(self, images):
        # import pdb; pdb.set_trace()
        image_features_clip_raw = self.get_model().get_vision_tower(load_model = "clip")(images)
        self.clip_output_raw = image_features_clip_raw
        image_features_clip = self.get_model().mm_projector(image_features_clip_raw)
        return image_features_clip


    def encode_images_withdino(self, images):
        # import pdb; pdb.set_trace()
        image_features_dino_raw = self.get_model().get_vision_tower(load_model = "dino")(images)
        self.dino_output_raw = image_features_dino_raw
        image_features_dino = self.get_model().dino_mm_projector(image_features_dino_raw)
    ...
    def prepare_inputs_labels_for_multimodal_withdino(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            # # 保存 CLIP 和 DINO 的输出
            # self.clip_output = image_features_clip
            # self.dino_output = image_features_dino
