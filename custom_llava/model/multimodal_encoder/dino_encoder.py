...
class DINOVisionTower(nn.Module):
    ...
    #         image_features = []
    #         for image in images:
    #             image_forward_out, hidden_states = self.vision_tower.forward_features(image.to(device=self.device, dtype=self.dtype).unsqueeze(0))
    #             image_feature = self.feature_select(image_forward_out).to(image.dtype)
    #             image_features.append(image_feature)
    #             hidden_states_list.append(hidden_states)
    #     else:
    #         image_forward_outs, hidden_states = self.vision_tower.forward_features(images.to(device=self.device, dtype=self.dtype))
    #         image_features = self.feature_select(image_forward_outs).to(images.dtype)
    #         hidden_states_list.append(hidden_states)

    #     return image_features,hidden_states_list
